...
public class ESP extends Module {
    ...
    private final Setting<List<Item>> items = sgItems.add(new ItemListSetting.Builder()
        .name("items")
        .description("Items to render ESP for.")
        .build()
    );

    private final Setting<ESPItemData> defaultItemConfig = sgItems.add(new GenericSetting.Builder<ESPItemData>()
    ...
    private final Setting<Map<Item, ESPItemData>> itemConfigs = sgItems.add(new ItemDataSetting.Builder<ESPItemData>()
        .name("item-configs")
    ...
    private final Setting<Boolean> itemTracers = sgItems.add(new BoolSetting.Builder()
        .name("item-tracers")
        .description("Render tracer lines for items.")
        .defaultValue(false)
        .build()
    );
    ...
    private void onTickRainbow() {
        ...
        defaultItemConfig.get().tickRainbow();
        for (ESPItemData itemData : itemConfigs.get().values()) itemData.tickRainbow();
    }
    ...
    private void drawBoundingBox(Render3DEvent event, Entity entity) {
        ...
        if (mode.get() == Mode.Wireframe) {
            WireframeEntityRenderer.render(event, entity, 1, sideColor, lineColor, shape);
        }
        ...
        if (entity instanceof ItemEntity itemEntity && itemTracers.get()) {
            ...
            if (itemData.tracer) {
                ...
                event.renderer.line(mc.player.getX(), mc.player.getY() + mc.player.getEyeHeight(mc.player.getPose()), mc.player.getZ(), x, y, z, itemData.tracerColor);
            }
        }
    }
    ...
    public boolean shouldSkip(Entity entity) {
        ...
        if (entity instanceof ItemEntity itemEntity) {
            return !items.get().contains(itemEntity.getStack().getItem());
        }
