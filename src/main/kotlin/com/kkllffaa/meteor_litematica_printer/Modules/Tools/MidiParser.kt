...
class MidiParser : Module(Addon.TOOLS, "midi-parser", "Parses MIDI files.") {
    ...
    private val midiFilePath: Setting<String> = sgGeneral.add<String?>(
    ...
    private val parseMidi: Setting<Boolean?> = sgGeneral.add<Boolean?>(
        BoolSetting.Builder()
            .name("parse-midi")
            .description("Click to parse the MIDI file.")
            .defaultValue(false)
            .onChanged(Consumer { value: Boolean? -> this.onParseMidiChanged(value!!) })
            .build()
    )
