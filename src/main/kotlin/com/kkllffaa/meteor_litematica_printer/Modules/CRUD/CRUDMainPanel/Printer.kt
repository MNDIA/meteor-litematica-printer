...
import com.kkllffaa.meteor_litematica_printer.Modules.CRUD.AtomicSettings.InteractSettings.Companion.interactWithBlock
...
class Printer : Module(Addon.CRUD, "litematica-printer", "Automatically prints open schematics") {
    ...
    private val sgRendering: SettingGroup = settings.createGroup("Rendering")

    private val sgCache: SettingGroup = settings.createGroup("Cache")

    //region General settings
    private val printing_range: Setting<Int?> = sgGeneral.add<Int?>(
        IntSetting.Builder()
            .name("printing-range")
    ...
    private val printing_delay: Setting<Int?> = sgGeneral.add<Int?>(
        IntSetting.Builder()
            .name("printing-delay")
    ...
    private val blocksPerTick: Setting<Int?> = sgGeneral.add<Int?>(
        IntSetting.Builder()
            .name("blocks/tick")
    ...
    private val 启用交互: Setting<Boolean?> = sgGeneral.add<Boolean?>(
        BoolSetting.Builder()
    ...
    private val firstAlgorithm: Setting<SortAlgorithm?> = sgGeneral.add<SortAlgorithm?>(
    ...
    private val secondAlgorithm: Setting<SortingSecond?> = sgGeneral.add<SortingSecond?>(
        EnumSetting.Builder<SortingSecond?>()
            ...
            .visible(IVisible { firstAlgorithm.get()!!.applySecondSorting })
            .build()
    )

    //endregion
    //region Whitelist settings
    private val whitelistenabled: Setting<Boolean?> = sgWhitelist.add<Boolean?>(
        BoolSetting.Builder()
    ...
    private val whitelist: Setting<MutableList<Block?>?> = sgWhitelist.add<MutableList<Block?>?>(
        BlockListSetting.Builder()
            .name("whitelist")
            .description("Blocks to place.")
            .visible(IVisible { whitelistenabled.get()!! })
            .build()
    )

    //endregion
    //region Rendering settings
    private val renderBlocks: Setting<Boolean?> = sgRendering.add<Boolean?>(
        BoolSetting.Builder()
    ...
    private val fadeTime: Setting<Int?> = sgRendering.add<Int?>(
        IntSetting.Builder()
            .name("fade-time")
            ...
            .visible(IVisible { renderBlocks.get()!! })
            .build()
    )

    private val colour: Setting<SettingColor?> = sgRendering.add<SettingColor?>(
    ...
    private val enableCache: Setting<Boolean?> = sgCache.add<Boolean?>(
        BoolSetting.Builder()
            .name("enable-cache")
    ...
    private val cacheSize: Setting<Int?> = sgCache.add<Int?>(
        IntSetting.Builder()
            .name("cache-size")
            ...
            .visible(IVisible { enableCache.get()!! })
            .build()
    )

    private val cacheCleanupInterval: Setting<Int?> = sgCache.add<Int?>(
        IntSetting.Builder()
    ...
    private val placed_fade: MutableList<Pair<Int?, BlockPos?>?> = ArrayList<Pair<Int?, BlockPos?>?>()
    ...
    private val pendingInteractions: MutableMap<BlockPos?, Int?> = HashMap<BlockPos?, Int?>()
    ...
    private fun onTick(event: TickEvent.Post?) {
        ...
        placed_fade.forEach(Consumer { s: Pair<Int?, BlockPos?>? -> s!!.setLeft(s.getLeft()!! - 1) })
            ...
            BlockIterator.register(
                ...
                BiConsumer { pos: BlockPos?, blockState: BlockState? ->
                    ...
                    if (启用交互.get() && !pendingInteractions.containsKey(pos)) {
                        val requiredInteractions = InteractSettings.calculateRequiredInteractions(
                            required,
                            blockState!!
                        )
                        if (requiredInteractions > 0) {
                            pendingInteractions.put(BlockPos(pos), requiredInteractions)
                        }
                    }
                    if (mc.player!!.getBlockPos().isWithinDistance(pos, printing_range.get()!!.toDouble())
                        && blockState!!.isReplaceable()
                        && required.getFluidState().isEmpty()
                        && !required.isAir() && blockState.getBlock() !== required.getBlock() && DataManager.getRenderLayerRange()
                            .isPositionWithinRange(pos)
                        && !mc.player!!.getBoundingBox().intersects(
                            Vec3d.of(pos),
            ...
            BlockIterator.after(Runnable {
                ...
                while (iterator.hasNext() && placed < blocksPerTick.get()!!) {
                    ...
                    if (remaining > 0) {
                        ...
                        val did = interactWithBlock(pos, toDo)
                        if (did > 0) {
                            timer = 0
                        }
                        placed += did
    ...
    enum class SortAlgorithm(val applySecondSorting: Boolean, algorithm: Comparator<BlockPos?>) {
        ...
        TopDown(true, Comparator.comparingInt<BlockPos?>(ToIntFunction { value: BlockPos? -> value!!.getY() * -1 })),
        DownTop(true, Comparator.comparingInt<BlockPos?>(ToIntFunction { obj: BlockPos? -> obj!!.getY() })),
        Nearest(
            false,
            Comparator.comparingDouble<BlockPos?>(ToDoubleFunction { value: BlockPos? ->
                if (MeteorClient.mc.player != null) Utils.squaredDistance(
